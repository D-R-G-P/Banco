...
if (isset($_GET['solicitudId']) && is_numeric($_GET['solicitudId'])) {
    ...
    if ($statement->execute()) {
        ...
        if ($result) {
            ...
            if ($currentState == 'archivo') {
                // Consulta SQL para actualizar el estado a "act"
                $updateQuery = "UPDATE solicitudes SET intervencion = 'si' WHERE id = :solicitudId";
                ...
                if ($updateStatement->execute()) {
                    $_SESSION['success_message'] = '<div class="notisContent"><div class="notis" id="notis">Solicitud desarchivada correctamente</div></div><script>setTimeout(() => {notis.classList.toggle("active");out();}, 1);function out() {setTimeout(() => {notis.classList.toggle("active");}, 2500);}</script>';
                    // Redireccionar a una página de éxito o mostrar un mensaje de éxito
                    header('Location: /Banco/app/search/expediente.php');
                    exit();
                } else {
                    $_SESSION['error_message'] = '<div class="notisContent"><div class="notiserror" id="notis">Error al desarchivar el estado: ' . implode(" ", $updateStatement->errorInfo()) . '</div></div><script>setTimeout(() => {notis.classList.toggle("active");out();}, 1);function out() {setTimeout(() => {notis.classList.toggle("active");}, 2500);}</script>';
                    // Redireccionar a una página de error o mostrar un mensaje de error
                    header('Location: /Banco/app/search/expediente.php');
                    exit();
                }
            } else {
                $_SESSION['error_message'] = '<div class="notisContent"><div class="notiserror" id="notis">La solicitud ya está desarchivada</div></div><script>setTimeout(() => {notis.classList.toggle("active");out();}, 1);function out() {setTimeout(() => {notis.classList.toggle("active");}, 2500);}</script>';
                // Redireccionar a una página de error o mostrar un mensaje de error
                header('Location: /Banco/app/search/expediente.php');
                exit();
            }
